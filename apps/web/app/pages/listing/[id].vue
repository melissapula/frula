      ...
      <div class="mt-6 grid gap-8 md:grid-cols-[1fr_320px]">
        <div>
          ...
          <div class="mt-6 flex flex-wrap gap-x-6 gap-y-3 border-y border-slate-200 py-5">
            ...
            <Stat v-if="listing.sqft" :value="formatNumber(listing.sqft)" label="sqft" />
            <Stat v-if="listing.lot_size" :value="`${listing.lot_size}`" label="acres" />
            <Stat v-if="listing.year_built" :value="listing.year_built" label="built" />
            <Stat v-if="listing.garage_stalls" :value="listing.garage_stalls" label="car garage" />
          </div>
