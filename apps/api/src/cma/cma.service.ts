...
export class CmaService {
  ...
  async generateEstimate(request: CmaRequestDto): Promise<CmaResult> {
    ...
    if (comps.length === 0) {
      throw new Error('Insufficient comparable sales data for this area. Try expanding your search radius.');
    }
  ...
  private calculateEstimate(request: CmaRequestDto, comps: Comp[]): {
    low_estimate: number;
    mid_estimate: number;
    high_estimate: number;
    confidence_score: number;
    ...
    const confidence = Math.min(95, Math.round(avgSimilarity * 0.7 + recencyBonus + (comps.length * 3)));
  ...
  private buildMethodologyText(comps: Comp[], request: CmaRequestDto): string {
    const avgAge = Math.round(
      comps.reduce((sum, c) => sum + c.days_since_sale, 0) / comps.length
    );
    return `Estimate based on ${comps.length} comparable sales within the area, ` +
